...
void perturbatePoints(std::vector<cv::Point2f> &img_pts,
                      float max_noise = 1.0) {
    for (auto &p : img_pts) {
        p.x += max_noise * frand();
        p.y += max_noise * frand();
...
int main(int argc, char **argv) {
    ...
    // Some camera parameters (no distortion params, we assume an undistorted
    // camera)
    ...
    // Define some object->camera transfomration (rotation in axis-angle
    // representation)
    cv::Mat_<double> r_vec_0 = (cv::Mat_<double>(3, 1) << M_PI / 6, M_PI / 6,
                                M_PI / 6),
                     t_vec_0 = (cv::Mat_<double>(3, 1) << 0, 0, 3.0), r_mat_0;

    // Convert the rotation in axis-angle representation to a 3x3 rotation
    // matrix
    ...
    // Recovere with RANSAC the homography matrix (3.0 is the RANSAC threshold
    // to define an inlier) inlier mask H is a vector with dimension equal to
    // the number of correspondences: each element is set to a value greater
    // than zero only if the corresponding correspondence represents an inlier
    cv::Mat H = cv::findHomography(scene_pts, img_pts_0, cv::RANSAC, 3.0,
                                   inlier_mask_H);

    // Count the inliers for the homography model
    int n_inliers_H = 0;
    for (int r = 0; r < inlier_mask_H.rows; r++)
        if (inlier_mask_H.at<unsigned char>(r)) n_inliers_H++;

    std::cout << "Num inliers H : " << n_inliers_H << std::endl;

    // Define e relative camera motion
    cv::Mat_<double> rel_r_vec = (cv::Mat_<double>(3, 1) << -M_PI / 24,
                                  -M_PI / 24, -M_PI / 24),
                     rel_t_vec = (cv::Mat_<double>(3, 1) << 0.2, 0, 0),
                     rel_r_mat, r_mat_1, t_vec_1;
    ...
    cv::projectPoints(scene_pts, r_mat_1, t_vec_1, K,
                      /*No dist coeff*/ cv::Mat(), img_pts_1);
    ...
    H = cv::findHomography(img_pts_0, img_pts_1, cv::RANSAC, 3.0,
                           inlier_mask_H);

    std::vector<cv::Mat> rotations, translations, normals;

    // Extracts the relative camera motion between the two views
    // The decomposeHomographyMat() function may return up to four mathematical
    // solution sets.
    ...
    // Print all the extracted solution: one should be quite close to the ground
    // truth, up to a scalar factor
    for (int i = 0; i < rotations.size(); i++) {
        cv::Mat rvec_decomp;
        cv::Rodrigues(rotations[i], rvec_decomp);
        std::cout << "r[" << i << "] : " << std::endl
                  << rvec_decomp << std::endl;
    ...
    cv::projectPoints(scene_pts, r_mat_0, t_vec_0, K,
                      /*No dist coeff*/ cv::Mat(), img_pts_0);
    ...
    // Use the PnP + RANSAC algorithms to estimate the transformation given the
    // set of 3D-2D correspondences
    cv::Mat est_r_vec, est_r_mat, est_t_vec;
    cv::solvePnPRansac(scene_pts, img_pts_0, K, cv::Mat(), est_r_vec,
                       est_t_vec);

    // Print both the ground truth and the estimated transformations: they
    ...
    // "Normalize" the points (i.e., represents their coordinates in the
    // canonical camera)
    ...
    // Estimate the essential matrix between the two views, along with the
    // corresponding inlier mask
    cv::Mat inlier_mask_E;
    cv::Mat E = cv::findEssentialMat(und_img_pts_0, und_img_pts_1,
                                     cv::Mat_<double>::eye(3, 3), cv::RANSAC,
                                     0.995, 0.01, inlier_mask_E);

    // Count the inliers for the essential model
    int n_inliers_E = 0;
    for (int r = 0; r < inlier_mask_E.rows; r++)
        if (inlier_mask_E.at<unsigned char>(r)) n_inliers_E++;

    std::cout << "Num inliers E : " << n_inliers_E << std::endl;

    // Recover relative camera rotation and translation from an estimated
    // essential matrix, by usinf the inlier mask as input/output parameter
    cv::recoverPose(E, und_img_pts_0, und_img_pts_1, K, est_r_mat, est_t_vec,
                    inlier_mask_E);

    // Print both the ground truth and the estimated transformations: they
    // should be quite similar, up to a scalar factor although often the
    // estimate is not so good due to poins noise and the small number of points
