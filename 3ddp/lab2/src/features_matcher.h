...
class FeatureMatcher {
   public:
    ...
    void setImagesNames(const std::vector<std::string>& images_names) {
        images_names_ = images_names;
    };

    // Extract from each image a set of salient points and compute descriptors
    void extractFeatures();

    // Perform exhaustive matching between features descriptors, and internally
    // store the results
    void exhaustiveMatching();

    // Write the results to file. If normalize_points is true, normalize
    // observations as seen by the canonical camera
    void writeToFile(const std::string& filename, bool normalize_points) const;

    //  // Read from file (used for debug)
    //  void readFromFile ( const std::string& filename, bool load_colors =
    //  false );
   ...
   private:
    // Read from file an image and undistort it, possibly by rescaling the focal
    // length (see the focal_scale parameter of the constructor)
    cv::Mat readUndistortedImage(const std::string& filename);

    // Get a single, unique ID from a pair [position ID, feature ID] (used as
    // hash)
    uint64_t poseFeatPairID(int pose_id, int feat_id) {
        return static_cast<uint64_t>(pose_id) | static_cast<uint64_t>(feat_id)
                                                    << 32;
    };

    // Add the matches between two images
    void setMatches(int pos0_id, int pos1_id,
