...
void FeatureMatcher::exhaustiveMatching()
{
  for (int i = 0; i < images_names_.size() - 1; i++)
  {
    for (int j = i + 1; j < images_names_.size(); j++)
    {
      ...
      /////////////////////////////////////////////////////////////////////////////////////////
      // matcher initialization: because AKAZE matcher is used, NORM_HAMMING is used as normType, as suggested in OpenCV doc
      cv::Ptr<cv::BFMatcher> matcher = cv::BFMatcher::create(cv::NORM_L2);

      // matches are computed and their relative points are stored in their relative data structure
      ...
      cv::findEssentialMat(src_points, dst_points, new_intrinsics_matrix_, cv::RANSAC, 0.999, 1.0, inlier_mask);
      for (int id_match = 0; id_match < src_points.size(); id_match++)
      {
        if (inlier_mask.at<uchar>(id_match))
        {
      ...
      // choose the best result, in terms of number of inliers, as final result --> 5 as threshold
      if (inlier_E.size() >= inlier_H.size() && inlier_E.size() > 5)
      {
        std::copy(inlier_E.begin(), inlier_E.end(), std::back_inserter(inlier_matches));
      }
      else if (inlier_H.size() > 5)
      {
        std::copy(inlier_H.begin(), inlier_H.end(), std::back_inserter(inlier_matches));
      }
      else
      {
        inlier_matches = {};
